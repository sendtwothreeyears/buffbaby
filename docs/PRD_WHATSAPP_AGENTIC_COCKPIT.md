# PRD: WhatsApp Agentic Development Cockpit
...
_WhatsApp is the world's most popular messaging app — 2B+ users, already installed on most smartphones. Rich formatting (monospace code blocks, clickable links), reliable delivery (in-order, read receipts), no additional app to download for most users. The engineer sends a WhatsApp message and gets a direct line to their own Claude Code instance running in the cloud: status updates as formatted text, code diffs as images, app previews as screenshots, approvals as simple replies. The conversation thread becomes the project log._

**Who** is affected:
Software engineers who already use Claude Code and agentic workflows. Specifically, engineers in bootcamps, startups, and teams who want to stay productive from anywhere — commutes, couches, coffee shops — without carrying a laptop. They need only a phone with WhatsApp installed. No laptop, no Mac, no personal server, no hardware of any kind. iPhone or Android — it doesn't matter.
...
- Every "solution" requires downloading another app. Engineers don't want another app — they want the tools to meet them where they already are. WhatsApp is already on most smartphones.
...
- The gap is clear: no tool puts an engineer's own Claude Code instance behind the messaging app that 2B+ people already have installed — WhatsApp.
...
- Every competitor requires downloading a new app or creating an account on a third-party platform. WhatsApp is already installed on most smartphones — minimal friction to start.

---

## Core Differentiator: Near-Zero-Install, Universal Access

Every competing product requires the user to download a new app or create an account on a new platform:
...
**This product requires: WhatsApp (already installed on 2B+ devices).** iPhone, Android — if they have a smartphone, they almost certainly have WhatsApp. No new download, no new account creation, minimal onboarding friction. The activation energy is "send a WhatsApp message."

This is not just a transport choice — it's the core product thesis. WhatsApp is the world's most popular messaging app, with rich formatting support (monospace code blocks, bold/italic, clickable links), reliable in-order delivery, read receipts, and a built-in audit trail.
...
| Goal | Metric | Target |
|------|--------|--------|
| Engineers can use Claude Code via WhatsApp | Any message the user sends is forwarded to their Claude Code instance | 100% of Claude Code capabilities accessible via WhatsApp |
| Reduce time-to-action on agent output | Time from agent completion to engineer review/approval | < 60 seconds (WhatsApp notification is instant) |
| App preview is visible without leaving WhatsApp | Engineer receives app screenshots (mobile + desktop viewport) as media messages | Screenshots delivered within 10 seconds of code change |
| Code review is viable on a phone | Engineer can review syntax-highlighted diff images via WhatsApp media and reply to approve/reject | Diff images are readable when zoomed, cover all changed files |
| Engineers adopt WhatsApp as a real workflow surface | % of agentic sessions initiated via WhatsApp (among active users) | 40%+ within 3 months |
| Conversation thread serves as project log | Engineers can scroll back through WhatsApp to see full session history | All interactions, responses, and images preserved in thread |
| Cross-platform from day one | Works on both iPhone and Android | 100% feature parity across platforms |
| Zero-new-app experience | Engineer never needs to leave WhatsApp during a workflow | 95%+ of interactions are text replies or image review within the thread |

**Anti-goals** (what we are NOT optimizing for):
- We are NOT building a native app. WhatsApp is the entire user-facing interface. Diffs are images. Previews are screenshots. Approvals are text replies. (A minimal onboarding website exists only for one-time signup — users never return to it.)
...
- We are NOT replacing the desktop experience. WhatsApp is a complementary surface for when you're away from your laptop.
...
- We are NOT requiring the engineer to download a new app or leave their WhatsApp thread. Everything happens in WhatsApp.
...
- Key needs: Access their Claude Code instance from their phone. Send commands, invoke their skills, see app previews, review diffs, approve deployments. Stay productive without a laptop — all within WhatsApp, an app they already have.
...
- Key needs: Monitor multiple agent sessions, review PRs created by agents, approve deployments. Could participate via WhatsApp group chats tied to projects.
...
- [ ] Twilio WhatsApp integration: service uses a Twilio WhatsApp Sandbox (dev) or approved WhatsApp Business number (production). Engineer sends a WhatsApp message. Twilio webhooks route to the relay server. Responses sent back via Twilio API.
...
- [ ] Thin relay server: lightweight Node.js server that receives Twilio webhooks, authenticates the user by phone number, looks up their VM, forwards the message text to Claude Code on that VM, and sends responses back via Twilio WhatsApp API. This is the platform's only custom code — Claude Code does all the real work.
- [ ] Phone → VM routing: relay maps each registered phone number to the user's Fly.io VM. Incoming WhatsApp message is forwarded to the correct VM. The relay is agnostic to what the user sends — it forwards everything as-is.
- [ ] Always-on cloud VM per user: Fly.io Machines running the Docker base image, one per user, always on — no cold starts, no hibernation. Engineer sends a message, gets an immediate response.
...
- [ ] Onboarding website: sign-up page where users enter phone number, connect GitHub (OAuth), and enter API keys (BYOK). Submitting the form triggers VM provisioning. User receives a welcome WhatsApp message confirming their instance is ready.
...
- [ ] Local development mode: relay server + Docker container run on the developer's Mac. ngrok exposes the relay to Twilio webhooks. Hardcoded config (`.env`). Same code, same behavior as production — only the host changes. Cost: free (Twilio WhatsApp Sandbox is free for development).

_WhatsApp ↔ Claude Code bridge — the core transport:_
- [ ] Text input forwarding: engineer sends any WhatsApp message and it's forwarded to Claude Code on their VM. This includes natural language ("add dark mode"), skill invocations (whatever skills the user has configured), or any other input. The relay doesn't interpret the content — it forwards.
- [ ] Agent output as WhatsApp message: Claude Code responses sent back as WhatsApp replies.
- [ ] Image output as WhatsApp media: when Claude Code generates images (screenshots, diffs, etc.), the relay fetches them from the VM and sends via Twilio WhatsApp API.
- [ ] Images served from VM: screenshots and diff images are generated and served from the user's VM. The relay fetches them via HTTP and sends via Twilio WhatsApp API.
...
- [ ] Dictation support: engineer uses their phone's native keyboard dictation (iOS/Android built-in) to speak commands. By the time the WhatsApp message reaches the service, it's already text. No voice infrastructure needed on our end.

_User-side capabilities — what the user's Claude Code setup provides (not platform features):_

The platform is agnostic to what the user does on their VM. Whatever Claude Code can do on a desktop, it can do via WhatsApp. The user brings their own:
...
- Skill invocation (e.g., if the user has a `/ship` skill, they send "/ship add dark mode")
...
- [ ] Composite diff images: when > 5 files change, batch into a single image to reduce message count. Option to "Reply 'show [filename]' for individual diffs."
- [ ] CI/CD status: relay listens for GitHub Actions webhooks and sends build/deploy status as a WhatsApp message.
...
- [ ] WhatsApp group chats: tie a group chat to a project — multiple engineers see agent activity, can issue commands.
- [ ] Multi-project management: manage multiple repos/VMs from different WhatsApp chats (one chat per project).
- [ ] Multi-channel expansion: add Telegram, Discord, Slack as additional transport layers. Same backend, different channels.
- [ ] Native app: a dedicated app for richer interaction — built on top of the same backend.

### Out of Scope (Non-Goals)

- A native app in V1. WhatsApp is the entire interface. A native app is a P2 consideration, built on the same backend.
...
- Offline mode. This product requires an internet connection to send/receive WhatsApp messages.
- Self-hosted VM management. V1 uses a managed cloud provider. Users don't manage infrastructure — they sign up, get a VM, and message.
- Opinionated workflows. The platform doesn't dictate what skills or workflows users should have. It provides the WhatsApp bridge; users bring their own Claude Code setup.
- Real-time streaming inside WhatsApp. WhatsApp delivers complete messages, not character-by-character streams. Agent progress is sent as periodic status updates ("Phase 2 of 11 complete...").
- OpenClaw integration in V1. The architecture is a thin relay server talking directly to Claude Code. OpenClaw could be considered in the future if multi-channel expansion (Telegram, Discord, Slack) becomes a priority.

---

## User Flows

_Note: The flows below illustrate the WhatsApp experience using example Claude Code skills and workflows. The specific skills shown (e.g., a "ship" workflow, a multi-agent investigation) are examples of what a user might have configured in their own Claude Code setup. The platform is agnostic — it forwards text and delivers images. What happens on the VM is determined by the user's Claude Code configuration._

**Flow 1: Onboarding**

1. Engineer visits the signup page
2. Enters phone number and joins the WhatsApp Sandbox (sends join code)
...
6. Receives a WhatsApp message: "You're set up. Send me anything to start."
7. Done. The engineer now has their own always-on Claude Code instance accessible via WhatsApp.

**Flow 2: Start working**
1. User sends WhatsApp message: "clone authentic-frontend and start the dev server on main"
...
4. System sends media: [app-screenshot-mobile.png] [app-screenshot-desktop.png]
5. User is ready to work — never left WhatsApp

**Flow 3: Run a workflow (example: user has a "ship" skill)**
1. User sends WhatsApp message: "/ship add a dark mode toggle to the settings page"
2. System replies: "Starting /ship workflow. I'll message you at each milestone."
...
5. System sends diff images as media: [diff-theme.ts.png] [diff-SettingsPage.tsx.png] [diff-ThemeContext.tsx.png] [diff-dark-mode.test.ts.png]
6. System sends updated app screenshots as media: [app-after-mobile.png] [app-after-desktop.png]
...
11. System sends staging screenshot as media: [staging-screenshot.png]
...
2. Phone's native dictation converts speech to text. WhatsApp message is sent as plain text.
...
8. System sends media: [diff-paymentService.ts.png]
...
12. System sends media: [app-checkout-fixed.png]

**Flow 5: Review code changes (example: user has a multi-agent review skill)**
1. User receives WhatsApp message: "Review complete. 14 findings across 6 agents."
2. System sends review summary: "3 blockers | 5 important | 6 suggestions"
3. System sends media: [review-summary.png] — a visual summary card showing findings grouped by severity
...
5. System sends diff images for each blocker as media: [blocker-1-auth.ts.png] [blocker-2-form.tsx.png] [blocker-3-query.ts.png]
6. User reviews images (pinch-to-zoom), then replies: "fix blockers"
7. System replies: "Fixing 3 blockers... Done. All tests passing."
8. System sends updated diff images showing the fixes as media
9. User replies: "approve"
...
2. System sends media: [app-settings-mobile.png] [app-settings-desktop.png]
3. User sends: "click the dark mode toggle"
4. System sends media: [app-settings-dark-mode-on.png]
5. User sends: "looks good. now show me the login page in dark mode"
6. System sends media: [app-login-dark-mode-mobile.png] [app-login-dark-mode-desktop.png]
...
3. System sends media: [diff-loginValidation.ts.png]
...
3. System sends media: [app-current-state.png]
4. User sends: "stop session"
5. System replies: "Session stopped. Resume anytime with 'resume authentic-frontend'."

**Edge cases to handle:**
- VM goes down or restarts — WhatsApp message: "Your session was interrupted. Last state saved at commit abc123. Reply 'resume' to pick up where you left off."
- Network interruption on phone — agent keeps working on VM. All WhatsApp updates queue in Twilio and deliver when connection resumes.
- Agent gets stuck (thrashing detection) — WhatsApp message: "Agent may be stuck (3 failed attempts on the same issue). Reply 'fresh' to spawn a fresh agent, or 'stop' to cancel."
...
- WhatsApp message limit (4096 chars) — long responses are split into multiple messages. WhatsApp supports monospace code blocks (triple backtick) for formatted output.

---

## Technical Considerations

- **Performance:** WhatsApp text responses must be sent within 5 seconds of agent output. Media messages (screenshots and diff images) must be generated and sent within 10 seconds of code changes (Playwright capture + image compression + Twilio delivery). VM is always-on — no provisioning delay.
- **Image quality:** Diff images must be high-resolution enough to read code when pinch-to-zoomed on a phone (minimum 2x retina, ~750px wide for mobile viewport). App screenshots must capture at both mobile (390px) and desktop (1440px) viewports. Images must be compressed appropriately: PNG for diffs (sharp text), JPEG for app screenshots. WhatsApp supports media up to 16MB.
- **Messaging cost:** WhatsApp messaging via Twilio is significantly cheaper than SMS/MMS. A heavy workflow (8 diff images + 2 screenshots) costs ~$0.125 total. Composite images (batching multiple diffs into one image) reduce message count when > 5 files change. Long-term, the `ImageStore` abstraction allows switching to hosted links (message + URL) to reduce media costs further.
- **Security:** All communication between the relay server and VMs must be encrypted (TLS/WSS). API keys for Claude, Codex, Gemini must never leave the VM. GitHub tokens use OAuth with minimal scopes. WhatsApp authentication: only registered phone numbers can issue commands (allowlist matched against Twilio's `From` field). Twilio webhook signature validation on all incoming requests.
- **Scalability:** Each user gets their own always-on VM. The relay server is stateless and horizontally scalable. VMs are isolated per user — no cross-contamination. Twilio handles WhatsApp delivery at any scale.
- **Integrations:** Twilio (WhatsApp), Claude Code CLI (headless mode), Playwright MCP (screenshot capture + page interaction), GitHub (repos, PRs, Actions), Codex CLI (optional), Gemini CLI (optional), Vercel / Railway / Fly.io (deployment).
...
- **WhatsApp-specific constraints:** WhatsApp supports basic formatting (bold, italic, monospace code blocks via triple backtick, clickable links). 4096-character message limit. 1 media attachment per message (multiple images = multiple messages). 16MB media limit. 24-hour session window — system can only reply within 24 hours of the user's last message. Sandbox requires a join code before first use. Delivery is reliable and in-order (no carrier variability).
...
│                    TWILIO                              │
│                                                       │
│  - Receives WhatsApp messages from engineer            │
│  - Webhooks to relay server                           │
│  - Sends WhatsApp responses back to engineer          │
│  - Handles message queuing, delivery, retry           │
│  - WhatsApp Sandbox (dev) or Business number (prod)   │
│                                                       │
└──────────────────┬────────────────────────────────────┘
                   │ HTTPS webhook
                   ▼
...
│  │ - Relay fetches images, sends via Twilio API    │  │
...
─────────────────────────        ──────────────────────────
- A phone with WhatsApp         - Twilio WhatsApp integration
- Internet connection           - Relay server (~200 lines)
...
- WhatsApp message in → forward to user's Claude Code instance
- Claude Code text out → WhatsApp message back to user
- Claude Code generates image → fetch from VM, send as WhatsApp media
...
──────────────────────────             ────────────────────────
- Twilio WhatsApp integration         - Their Claude Code setup
...
- Image pipeline (VM → WhatsApp)      - API keys (BYOK)
- Onboarding website + database       - Their workflows and processes
- Phone → VM routing                  - Project-specific .mcp.json
```

The relay is deliberately dumb. It doesn't parse skill names, interpret commands, or understand workflow state. It forwards messages and delivers images. All intelligence lives in Claude Code on the user's VM.
...
docker build -t whatsapp-cockpit-vm .
docker run -p 3000:3000 whatsapp-cockpit-vm
...
**The user never sees Docker.** Docker is a development and deployment tool — it packages everything into a portable container. Users send a WhatsApp message. They don't know Docker, Fly.io, or VMs exist.

### Local Development Setup

For developing and testing the product, everything runs on your Mac. Cost: free (Twilio WhatsApp Sandbox is free for development).
...
- Twilio account with WhatsApp Sandbox enabled (free for development)
- Your own API keys for Claude, Codex, Gemini

**What happens:** You send a WhatsApp message to the Twilio Sandbox number from your phone. Twilio webhooks to ngrok, which tunnels to your local relay server. The relay forwards the command to Claude Code running inside the Docker container on your Mac. Claude Code does the work, takes screenshots via Playwright MCP, and the relay sends the result back via Twilio WhatsApp API to your phone.
...
**Development mode:** For building and testing the platform. The relay reads from `.env`. One phone number, one Docker container. You send a WhatsApp message, it works.

**Production mode:** Users sign up on the website, get a VM provisioned automatically, and start messaging via WhatsApp. The relay routes their phone number to their VM.
...
2. Backend calls Fly.io Machines API:
   POST https://api.machines.dev/v1/apps/whatsapp-cockpit/machines
   {
     "image": "registry.fly.io/whatsapp-cockpit-vm:latest",
     "config": {
       "env": {
...
5. User receives welcome WhatsApp message: "You're set up. Send me a message to start."
6. Every future WhatsApp message from that phone number routes to their VM.
```
...
The relay code, the Docker image, the Twilio WhatsApp configuration, the Claude Code configuration — all identical between local and production. The only thing that changes is where the containers run.
...
- Engineers are willing to review code diffs via pinch-to-zoom on syntax-highlighted images in WhatsApp
- Syntax-highlighted diff images at retina resolution are readable enough for meaningful code review on a phone screen
- WhatsApp delivery latency is acceptable for development workflows (typically < 2 seconds for text, < 5 seconds for media)
...
- Engineers prefer using an interface they already have (WhatsApp) over downloading a new app
- Claude Code CLI can be reliably driven headlessly (`claude -p` / `claude --headless`) for all user workflows. The relay forwards the user's message as the prompt — Claude Code handles everything else natively.
...
- WhatsApp can handle the volume of media messages generated during a development session (estimated 20-50 images per hour during active development) without throttling

**Constraints** (hard limits):
- WhatsApp supports basic formatting (bold, italic, monospace code blocks) but not full markdown or syntax highlighting. Visual content (diffs, app previews, review summaries) should be rendered as images for readability.
- WhatsApp media limit is 16MB per attachment. 1 media attachment per message — multiple images require multiple messages.
- WhatsApp message limit is 4096 characters. Long responses must be split into multiple messages.
- 24-hour session window: the system can only send messages within 24 hours of the user's last message. After 24 hours, a pre-approved template message is required to re-engage.
- Twilio WhatsApp Sandbox requires a join code before first use (development). Production requires WhatsApp Business API approval.
...
- No real-time streaming. WhatsApp delivers complete messages. Progress updates are periodic ("Phase 2 of 11 complete...").
- WhatsApp messages are delivered reliably and in-order (no carrier variability like SMS).

**Dependencies:**
- Docker (for building and running the VM container — development and production)
- Twilio account with WhatsApp Sandbox (development) or WhatsApp Business API approval (production)
...
| Does the 24-hour WhatsApp session window cause issues for long-running workflows? | Emmanuel | Open | Test workflows that span multiple hours. May need template messages to re-engage after 24h of user inactivity. |
| Will WhatsApp rate limits affect high-volume development sessions? | Emmanuel | Open | Test sustained message volume (50+ messages/hour) via Twilio WhatsApp Sandbox and monitor for throttling. |
...
| What's the minimum viable pricing model that covers infrastructure costs? | Emmanuel | Decided | **$29/month** per user. Infrastructure cost ~$11-13/month for a power user (Fly.io VM $5 + Twilio WhatsApp ~$5-8 + relay $1). ~$16-18/month margin at power-user level, more for lighter users. WhatsApp is ~75% cheaper than SMS/MMS. |
...
| What happens when a long-running workflow (30+ min) generates dozens of status updates and images? Will it overwhelm the WhatsApp thread? | Emmanuel | Open | Design message batching and summary patterns. Don't spam the user — send milestone updates, not every line of output. |
...
| How does the user configure their Claude Code setup on the VM? (clone their repo with .claude/skills/? Pre-configure during onboarding?) | Emmanuel | Open | Users need their custom skills, CLAUDE.md, and .mcp.json on the VM. Options: clone their config repo during onboarding, provide a "setup" command via WhatsApp, or let them configure via their first session. |

---

## Timeline & Milestones

### Phase A: Local Development (your Mac, free with Twilio Sandbox)

| Milestone | Target Date | Notes |
|-----------|-------------|-------|
| PRD approved | TBD | This document |
| Twilio account + WhatsApp Sandbox | TBD | Register, enable WhatsApp Sandbox (free for development) |
| WhatsApp echo prototype | TBD | Twilio → ngrok → local server → echo back. Send a test media message. Prove plumbing works. |
| Docker image v1 | TBD | Dockerfile with Claude Code CLI + Playwright MCP + Node.js + git + Chromium. Runs locally via `docker run`. |
| Relay server prototype | TBD | Twilio webhook → ngrok → relay → Claude Code in Docker → response back via WhatsApp. All on your Mac. |
| Screenshot pipeline | TBD | Claude Code uses Playwright MCP inside Docker to capture app screenshots, relay sends via WhatsApp media |
| Diff image pipeline | TBD | Syntax-highlighted diff rendering to PNG inside Docker, sent via relay as WhatsApp media |
| End-to-end local flow | TBD | Full loop on your Mac: send WhatsApp message to sandbox number → relay → Claude Code in Docker → run a workflow → diffs + screenshots → approve → PR created. All from your phone. |
...
| End-to-end production flow | TBD | Full loop: sign up → send WhatsApp message → VM auto-created → run a workflow → diffs + screenshots → approve → PR. Laptop closed the whole time. |
...
| Stability + polish | TBD | Error handling, edge cases, session management, image quality tuning, WhatsApp delivery testing |
| Cost model validated | TBD | Confirm unit economics: $29/month subscription covers ~$11-13/month infrastructure per power user (significant margin improvement vs SMS) |
...
- **Rollout approach:** Build locally first, deploy to Fly.io second, onboard users third. Docker-first development ensures zero friction between phases.
  ...
  **Local Development (your Mac):**
  - **Phase 1 (Echo):** Twilio → ngrok → local relay → echo back. Send a test media message. Proves WhatsApp plumbing works end-to-end.
  - **Phase 2 (Docker):** Build the Docker image with Claude Code + Playwright MCP. Run it locally. Relay talks to the local container.
  - **Phase 3 (Command):** Relay routes text commands to Claude Code in Docker. Agent responses come back as WhatsApp messages. Text-only, no images yet.
  - **Phase 4 (Screenshots):** Claude Code uses Playwright MCP inside Docker to capture app screenshots. Relay sends them as WhatsApp media.
  - **Phase 5 (Diff images):** Add syntax-highlighted diff rendering. Code changes rendered as PNG, sent as WhatsApp media alongside screenshots.
  ...
  **Scale & Polish:**
  - **Phase 10 (Beta):** Invite 10 bootcamp engineers. Monitor costs, reliability, WhatsApp media delivery.
  - **Phase 11 (Conversational navigation):** Engineer sends "show me the login page" → Claude Code navigates via Playwright MCP → sends screenshot via WhatsApp media.
  ...
  - **Phase 14 (Polish):** Session management, error handling, `/help`, `/history`, `/cost`, composite images, 24-hour session window handling.
...
- **Graduation criteria** (what must be true to proceed to wider launch):
  ...
  - Media messages are readable and arrive in correct order
...
- **Rollback plan:**
  ...
  - Twilio WhatsApp number can be deactivated — users simply stop receiving responses. No data loss. All code is safe in GitHub.
  - If WhatsApp proves untenable (API changes, cost issues), the same backend can power Telegram, Discord, Slack, or a lightweight native app — the transport is swappable, the backend is the same.
...
| Twilio WhatsApp Sandbox | Free | WhatsApp Sandbox is free for development |
| Your API keys (Claude, etc.) | Your existing keys | No additional cost |
| **Total to develop & test** | **Free** | Everything runs on your Mac with free Twilio Sandbox |

### Production Infrastructure Costs (Fly.io)

| Component | Provider | Cost | Notes |
|-----------|----------|------|-------|
| Twilio WhatsApp Business | Twilio | ~$0/month (pay-per-message) | No monthly number fee for WhatsApp |
| WhatsApp Business API approval | Twilio/Meta | Free (one-time process) | Required for production (non-Sandbox) use |
...
### Twilio WhatsApp Messaging Costs (Per Message)

| Message Type | Direction | Cost |
|-------------|-----------|------|
| WhatsApp text (session) | Outbound | ~$0.005 |
| WhatsApp text (session) | Inbound | Free |
| WhatsApp media (session) | Outbound | ~$0.005 |
| WhatsApp media (session) | Inbound | Free |
| WhatsApp template (outside 24h) | Outbound | ~$0.05 |

_Note: WhatsApp "session" messages (within 24h of user's last message) are significantly cheaper than SMS/MMS. Template messages (outside the 24h window) cost more but are rarely needed._

### Cost Per Workflow (Estimated)

| Workflow Type | Text messages | Media messages | Twilio cost |
|--------------|------------|------------|-------------|
| Heavy workflow (multi-phase implementation) | ~15 messages | ~10 images | ~$0.125 |
| Medium workflow (multi-agent investigation) | ~8 messages | ~3 images | ~$0.055 |
| Light workflow (quick commit + PR) | ~3 messages | ~2 images | ~$0.025 |
| Simple command ("show me the app", "status") | ~2 messages | ~1 image | ~$0.015 |
...
| Twilio WhatsApp messaging (~10 workflows/day) | ~$5-8 | Variable — ~75% cheaper than SMS/MMS |
| Relay server share | ~$1 | Amortized across all users |
| **Total infrastructure per user** | **~$11-13/month** | **Before AI API costs (user pays via BYOK)** |
...
| **Launch** | **$29/month** | Always-on VM + unlimited WhatsApp messaging. BYOK for AI APIs. ~$16-18/month margin at power-user level, even more for lighter users. |
...
| Item | Cost | Notes |
|------|------|-------|
| Twilio account + WhatsApp Business API | Free | One-time setup (approval process) |
...
| **Total to launch beta** | **~$180** | Before any revenue |

_Note: This is significantly cheaper than the iMessage approach (~$700-1,400 upfront) and even cheaper than the SMS approach (~$200). WhatsApp via Twilio eliminates phone number fees and A2P 10DLC registration costs._
...
- Twilio WhatsApp API integration (webhooks, message sending, WhatsApp Business API)
...
- WhatsApp Business API compliance (24-hour session windows, template message approval, sandbox-to-production migration)

**What excites me most about this project:**
- The radical simplicity — use an app you already have (WhatsApp). Send a message and you're coding.
...
- Upfront cost is ~$180 to launch a beta, not $700-1,400
- Proving that the phone can be a real engineering surface, not just a consumption device
- Making agentic workflows accessible to anyone with a phone and WhatsApp
