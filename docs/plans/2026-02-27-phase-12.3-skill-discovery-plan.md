...
When users clone a repo that has `.claude/skills/`, those skills appear in help and as Discord slash commands. Skills are invoked via Claude Code (freeform), not the command router — the command router only discovers and *displays* them.
...
- [x] Enhance `vm/skills.js` with description parsing (frontmatter + first paragraph)
- [x] Add cache TTL logic to skill scanning (5-minute expiry in `skills_cache` table)
- [x] Add `skills --refresh` command to relay command router
- [x] Add `GET /skills?refresh=true` parameter support on VM
- [x] Register core Discord slash commands on startup (help, status, clone, switch, repos)
- [x] Add `updateSlashCommands(skills[])` to Discord adapter
- [x] Handle `interactionCreate` event in Discord adapter
- [x] Implement deferred reply pattern for slash commands (>3s work)
- [x] Add autocomplete for `switch` command (list repos)
- [x] Add startup reconciliation (remove stale slash commands)
- [x] Enhance `help` output to include project skills section
- [x] Update relay skill cache to notify Discord adapter on change
...
- Discord slash commands persist on Discord's side after relay restart → need reconciliation on startup
