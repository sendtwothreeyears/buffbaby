...
- Phase 12.2 complete (output rendering — PR status benefits from smart formatting)
- Phase 12.3 complete (slash commands — PR/branch commands register as Discord slash commands)
...
**Logic:** Add an `isOnboarded()` check in the relay's action-command handler. If not onboarded and the command is freeform, prepend the welcome message. Only show once — set `config.onboarded = true` after first display.
...
📋 Core Commands
  ...
  help           — Show this help
  cancel/approve/reject — Control flow
```

## Implementation Tasks

- [x] Add `pr create`, `pr status`, `pr merge` to relay command router
- [x] Add VM endpoints: POST `/pr/create`, GET `/pr/status`, POST `/pr/merge`
- [x] Add `branch`, `checkout` to relay command router
- [x] Add VM endpoints: GET `/branch`, POST `/checkout`
- [x] Implement onboarding flow (detect first interaction, show welcome)
- [x] Store `onboarded` flag in SQLite config table
- [x] Register new slash commands in Discord adapter
- [x] Update help output with new commands
- [x] Update Phase 12.3's `updateSlashCommands()` to include PR/branch commands
