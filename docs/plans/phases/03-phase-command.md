...
**Done when:** You text "what is 2+2" to the Twilio number, Claude Code answers, you get the answer back via WhatsApp.

## What You Build

Connect the relay server (Phase 1) to the Docker container (Phase 2). The relay receives an incoming WhatsApp message via Twilio webhook, forwards the message body to Claude Code's HTTP API inside the Docker container, and sends Claude Code's text response back via WhatsApp through Twilio.

This is the core transport — WhatsApp message in, Claude Code processes, WhatsApp message out. The relay is a dumb pipe.

Deliverables:
- Relay server upgraded from echo to forwarding: receives WhatsApp message → POSTs to Docker container's `/command` endpoint → sends response via WhatsApp
- **Async webhook pattern:** Relay responds to Twilio's inbound webhook immediately with `200 OK` (empty TwiML `<Response></Response>`). Claude Code command runs asynchronously. Response sent as a new outbound WhatsApp message via Twilio REST API. This is required because Claude Code takes longer than Twilio's 15-second webhook timeout.
...
- Error handling: connection refused, 500, timeout from Docker container → user receives a sensible error message ("Something went wrong. Try again in a moment.")
...
- Response truncation at 4096 chars for WhatsApp message limit

## Tasks

- [x] Relay forwards incoming WhatsApp message to Claude Code HTTP API in Docker container and sends the response back via Twilio WhatsApp
...
- Claude Code may return long responses. WhatsApp has a 4096-char message limit — consider truncating very long responses with a note. Full "Reply 'more'" support is deferred to Phase 16 (UX Polish).
...
| Response truncation at 4096 chars | PASS | `server.js:153-155` — truncates with `[Response truncated]` suffix |
