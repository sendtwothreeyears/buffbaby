...
A Docker image containing the full VM runtime: Claude Code CLI, Playwright, Node.js, git, and Chromium. Inside the container, a thin HTTP API wrapper accepts a command string via POST, runs it through Claude Code in headless mode (`claude -p --dangerously-skip-permissions`), and returns a structured JSON response.

Deliverables:
- `Dockerfile` with Claude Code CLI, Playwright, Node.js, git, Chromium
...
- ~~`.mcp.json` pre-configured with Playwright MCP server~~ _(removed in 177f86f — Playwright used via Node.js API, not MCP)_
- `.env.example` documenting all required environment variables

## Tasks

- [x] Build Dockerfile with Claude Code CLI + Playwright + Node.js + git + Chromium, expose HTTP API that accepts a command and returns Claude Code headless output
  - Brainstorm: `/workflow:brainstorm Docker image design — Claude Code headless + Playwright + HTTP API wrapper`
...
- Playwright/Chromium needs `--no-sandbox` flag in Docker (Chromium sandboxing conflicts with Docker's own sandboxing).
- **Validate Playwright works:** After building the image, test with a prompt like "take a screenshot of https://example.com" to confirm Playwright can launch Chromium. Catch config issues now, not in Phase 4.
...
- ~~Plan document (`docs/plans/2026-02-25-feat-docker-vm-image-plan.md`) still references `.mcp.json` and old Dockerfile~~ — historical artifact, left as-is
- ~~Phase 4 notes assume `.mcp.json` exists~~ — resolved in Phase 4 implementation (uses Playwright Node.js API via `POST /screenshot`)
