...
**Done when:** You text a natural language command from your phone, Claude Code executes it, you receive progress updates as SMS, diff images and app screenshots as MMS, and can reply "approve" to create a PR on GitHub. All running on your Mac.
...
Deliverables:
- **Progress update streaming via callback:** The API wrapper on the container POSTs milestone updates to a callback URL on the relay (e.g., `POST /callback/:sessionId`). The relay sends each update as an SMS. This matches the relay's existing webhook-based architecture and avoids the complexity of WebSocket connections.
...
working ──(Claude Code sends milestone)──→ working (send SMS update)
working ──(Claude Code requests approval)──→ awaiting_approval
working ──(Claude Code finishes without approval)──→ idle
working ──(Claude Code errors)──→ idle (send error SMS)
...
- [ ] Wire progress update streaming — container POSTs milestones to relay callback endpoint, relay sends as SMS
