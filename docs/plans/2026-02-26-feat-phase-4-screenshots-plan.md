---
title: "feat: Phase 4 Screenshots — VM capture to WhatsApp delivery"
type: feat
status: completed
date: 2026-02-26
phase: 4
...
# Phase 4: Screenshots — VM Capture to WhatsApp Delivery

## Overview

Build a screenshot pipeline that lets Claude Code capture screenshots of a running dev server inside the Docker container and deliver them to the user's phone via WhatsApp. This is the first phase where **images flow from the VM to the phone**.

**Done when:** You text "show me the app" and receive a screenshot of the running app on your phone via WhatsApp.

## Problem Statement

The WhatsApp cockpit currently only returns text. Engineers need to **see** what their app looks like — diffs are images, previews are screenshots, UI bugs need visual confirmation. Phase 4 closes the image gap by building the pipeline from Playwright capture on the VM through to WhatsApp delivery on the user's phone.
...
User texts "show me the app"
  ...
  → Relay sends WhatsApp media via Twilio with mediaUrl
...
5. If still > 16MB after 1x DPR, fail with error

**Target:** < 300KB for mobile screenshots, < 600KB for desktop. WhatsApp supports up to 16MB media.
...
### Component 4: Relay WhatsApp Media Sending

**File:** `server.js`

Update the outbound message flow to support WhatsApp media when images are present.

```javascript
// server.js — updated sendMessage with media support
...
const mediaUrls = (data.images || [])
  .slice(0, 10)  // Twilio limit: 10 media URLs per message
  .map(img => `${PUBLIC_URL}${img.url}`);

await sendMessage(from, data.text, mediaUrls);
```

**Twilio WhatsApp media limit:** Max 10 media URLs per message. If > 10 images (unlikely), send only the first 10. This edge case does not need batching in Phase 4.

**Timing dependency:** After the relay sends the WhatsApp media message, Twilio asynchronously fetches images from the relay's public URL. Images must remain accessible for at least 15 minutes. The TTL cleanup (Component 5) ensures this.
...
- The screenshot is automatically sent to the user as a WhatsApp image — just confirm what you captured in your text response
...
<head>
  ...
  <title>WhatsApp Cockpit Test App</title>
  <style>
...
<body>
  <header>
    <h1>WhatsApp Cockpit Test App</h1>
  ...
  <main>
    ...
    <div class="card">
      <h2>About</h2>
      <p>This is a static test page served inside the Docker container. If you can see this as a WhatsApp screenshot on your phone, the Phase 4 pipeline is working end-to-end.</p>
    </div>
    <div class="card">
      <h2>Viewport</h2>
...
### Phase 4b: Relay Image Proxy + WhatsApp Media

**Files:** `server.js`

- [x] Add `GET /images/:filename` proxy route with filename validation
- [x] Update `sendMessage` to accept `mediaUrl` parameter
- [x] Update `processCommand` to read `data.images` and construct public media URLs
- [x] Add `[IMAGE_PROXY]` / `[MEDIA]` logging
- [ ] Test: send a command that produces an image, verify WhatsApp media arrives on phone
...
- [ ] End-to-end test: text "show me the app" → receive WhatsApp screenshot
...
- [x] Update `docs/plans/phases/04-phase-screenshots.md` — remove stale references, update to match implementation
...
- [ ] JPEG file size is < 16MB (target < 300KB mobile, < 600KB desktop)
- [ ] `/command` response `images` array contains screenshot metadata
- [ ] Relay `GET /images/:filename` proxies images from VM
- [ ] Relay sends WhatsApp media with `mediaUrl` when images are present
...
| JPEG too large after compression | Low | Medium | Iterative quality reduction, DPR fallback |
...
- Relay server: `server.js` (sendMessage at line 188-199, processCommand at line 111-148)
...
- `docs/solutions/developer-experience/sms-echo-server-twilio-ngrok-setup-20260225.md` — WhatsApp media size limits, logging prefix conventions
