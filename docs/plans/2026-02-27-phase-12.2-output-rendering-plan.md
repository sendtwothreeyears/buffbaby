...
- [ ] Create `vm/output-classifier.js` — output type detection
- [ ] Create `vm/web-views.js` — HTML generation, template rendering
- [ ] Create HTML templates (diff, code, log, base layout) — mobile-first
- [ ] Add `artifacts` table to SQLite schema (migration v2)
- [ ] Add `GET /view/:id` endpoint to VM server
- [ ] Create `/data/views/` on VM startup
- [ ] Update VM `/command` response to include `outputType` and `viewUrl` for long output
- [ ] Add `GET /view/:id` proxy endpoint to relay
- [ ] Update WhatsApp adapter `sendVMResponse` for web view links
- [ ] Update Discord adapter `sendVMResponse` for web view links
- [ ] Update Telegram adapter `sendVMResponse` for web view links
- [ ] Extend artifact cleanup to handle `/data/views/`
