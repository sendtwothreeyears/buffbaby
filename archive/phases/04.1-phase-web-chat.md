...
- `POST /chat` — JSON API endpoint (same as `/webhook` flow but no Twilio dependency)
...
| Sending a text message returns Claude Code's response | PASS | `POST /chat` (`server.js:63-84`) calls `forwardToVM(text)` — same function used by `/webhook` handler — and returns the JSON response. UI sends `fetch("/chat")` with JSON body (`index.html:271-274`). |
...
| Existing WhatsApp/Twilio endpoints unchanged | PASS | `git diff` confirms only additions: `path` import, `express.json()` middleware, `GET /` and `POST /chat` routes. Zero changes to `/webhook` handler, `processCommand`, `sendMessage`, or VM code. VM directory has zero diff. |
