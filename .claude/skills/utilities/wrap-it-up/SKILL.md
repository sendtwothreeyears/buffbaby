---
name: utilities:wrap-it-up
description: "Finalize work with quad-agent code review, smart deduplication, parallel fixes, and a final sanity check"
...
**Usage:** `/utilities:wrap-it-up` — Review, fix, commit, PR, and verify the current branch
...
**Guard rails:**
- If on the default branch, STOP and ask user which branch to work on
- Use the branch name as the identifier for review files
...
This ensures origin/$default_branch is current for accurate diffs.
...
Check the diff size:
```bash
git diff origin/$default_branch...HEAD | wc -l
```
...
- **Branch:** {branch name}
- **Story ID:** {STORY_ID}
- **Base:** origin/$default_branch at {merge-base hash}
...
**Claude Normal:**
```bash
claude -p "Read /tmp/wrap-review-normal-prompt.md and follow the instructions. Write your review to notes/CR-${BRANCH}-Claude-Normal-${REVIEW_TS}.md" --dangerously-skip-permissions
```

**Codex Normal:**
```bash
codex exec --full-auto --skip-git-repo-check "Read /tmp/wrap-review-normal-prompt.md and follow the instructions. Write your review to notes/CR-${BRANCH}-Codex-Normal-${REVIEW_TS}.md"
```

**Claude Critical:**
```bash
claude -p "Read /tmp/wrap-review-critical-prompt.md and follow the instructions. Write your review to notes/CR-${BRANCH}-Claude-Critical-${REVIEW_TS}.md" --dangerously-skip-permissions
```

**Codex Critical:**
```bash
codex exec --full-auto --skip-git-repo-check "Read /tmp/wrap-review-critical-prompt.md and follow the instructions. Write your review to notes/CR-${BRANCH}-Codex-Critical-${REVIEW_TS}.md"
```

Launch all four in parallel (in a single message, without `run_in_background`). Timeout after 5 minutes per agent.
...
1. Stage all changes (review files in `notes/` are gitignored and won't be committed):
   ```bash
   git add -A
   ```
...
3. Commit with comprehensive message:
   ...
   Fixes validated and applied by: Claude
...
git merge origin/$default_branch --no-commit --no-ff || echo "CONFLICTS"
